...
class TestZampAPAgentWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_construct_email_records_date_parsing(self, workflow_instance):
        """Test date parsing in construct_email_records function"""
        # Create a single test email with string date
        test_email = GmailMessage(
            id="msg1",
            thread_id="thread1",
            label_ids=["INBOX"],
            snippet="Test email",
            ...
            size_estimate=1000,
            raw=None,
            date="2024-03-20T10:00:00Z",  # Test string date format
            subject="Test Invoice",
            sender="vendor@example.com",
            recipient="ap@zamp.finance",
            body="Test body",
        ...
        with patch(
            ...
            # Mock the code executor activity
                    ...
                    [
                        ZampApAgentEmailSchema(
                            ...
                            **{"from": "vendor@example.com"},
                            received_at=datetime.datetime(2024, 3, 20, 10, 0),
                            storage_path="",
                            status=STATUS_UNPROCESSED,
                            created_at=datetime.datetime.now(),
                            updated_at=datetime.datetime.now(),
            ...
            # Call the method that uses construct_email_records
            result = await workflow_instance._get_email_records_and_ids([test_email])

            # Verify the date was parsed correctly
            assert result[0][0].received_at == datetime.datetime(2024, 3, 20, 10, 0)
